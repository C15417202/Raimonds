...
def refreshScreenFunction():
    ...
    cur = con.cursor()
    cur.execute("INSERT INTO data(CAN_ID, CAN_LEN, CAN_DATA) VALUES(%d, %d, %d) " %(can_id, can_dlc, data))
    con.commit()
    ...
    if(str(can_id) == "3"):
        MainWindow.speed.display(str(data))
    if(str(can_id) == "4"):
    ...
    if(str(can_id) == "7"):
        MainWindow.motorVoltageLeft.setText(str(data))
    if(str(can_id) == "8"):
        MainWindow.motorVoltageRight.setText(str(data))
    if(str(can_id) == "9"):
        MainWindow.motorCurrentLeft.setText(str(data))
    if(str(can_id) == "10"):
        MainWindow.motorCurrentRight.setText(str(data))
    if(str(can_id) == "11"):
